<template>
          ...
          <template slot-scope="scope">
            ...
            <el-row :class="['bdbottom',i1===0?'bdtop':'','vcenter']" v-for="(item1, i1) in scope.row.children" :key="item1.id">
              ...
              <el-col :span="5">
                ...
                <el-tag closable @close="deleteitem3(scope.row,item1.id)">{{ item1.authName }}</el-tag>
  ...
  @click="editRolemsg(scope.row)">编辑</el-button>
...
<!-- 分配权限对话框 -->
  ...
  -->
  <el-tree :default-expand-all="true" node-key="id" 
  show-checkbox :data="rightslist" ref="treeIdRef" :props="defaultProps"
...
export default {
  ...
  async  deleteitem3(role,item3id){
     const confirmResult=   await this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
       ...
       const {data:res}=await this.$http.delete(`roles/${role.id}/rights/${item3id}`)
       if(res.meta.status!=200){
  ...
  async allotrights(role){
    ...
    // node角色列表的数据，用node.children判断是否是三级节点，arr保存三级权限的id
     recursionItem3id(node,arr){
        //  如果node节点里没有children则把node.id保存到arr数组中
            ...
            this.recursionItem3id(item,arr))
     },
    //  监听关闭分配权限对话框的事件
     closeDialogEvent(){
   ...
   const {data:res} = await this.$http.post('roles',this.addRole)
    ...
    updaterole(){
      ...
      if(!valid) return
      // 发送修改请求
       const {data:res} = await this.$http.put('roles/'+this.editid,this.editinfo)
      if(res.meta.status!=200){
